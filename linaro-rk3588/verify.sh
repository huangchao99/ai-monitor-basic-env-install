...
[[ "${runtime_version}" == *'2.3.2'* ]] \
  ...
  || fail "expected RKNN Runtime 2.3.2, found: ${runtime_version:-unknown}"
...
[[ -e /dev/mpp_service || -e /dev/mpp-service ]] && pass "MPP device is present" || fail "MPP device is missing"
...
[[ -e /dev/ttyS4 ]] && pass "GPS serial device is present: /dev/ttyS4" || fail "GPS serial device is missing: /dev/ttyS4"
